...
router.get("/household/list", authentication, (req, res, next) => {
  ...
  pool.query(sql1, params1, (err, result1) => {
    ...
    let sql2 =
      "SELECT hid,householdId,householdName,gender,householdAddr,phone,idNum FROM zh_household "
    let params2 = []
    if (kw) {
      sql2 += " WHERE householdName LIKE ? "
      params2.push("%" + kw + "%")
    }
    sql2 += " ORDER BY hid ASC LIMIT ?, ?"
...
router.get("/household/details", authentication, (req, res, next) => {
  ...
  let sql = "SELECT * FROM zh_household WHERE hid=?"
 ...
 */
router.get("/household/update", authentication, (req, res, next) => {
  let aid = req.token.aid
